...
public abstract class PathNavigator {
   ...
   protected Path func_220971_a(BlockPos p_220971_1_, double p_220971_2_, double p_220971_4_, double p_220971_6_, int p_220971_8_, boolean p_220971_9_) {
      if (this.field_75515_a.posY < 0.0D) {
         return null;
         ...
         BlockPos blockpos = p_220971_9_ ? (new BlockPos(this.field_75515_a)).up() : new BlockPos(this.field_75515_a);
         int i = (int)(f + (float)p_220971_8_);
         IWorldReader iworldreader = new Region(this.world, blockpos.add(-i, -i, -i), blockpos.add(i, i, i));
         Path path = this.pathFinder.findPath(iworldreader, this.field_75515_a, p_220971_2_, p_220971_4_, p_220971_6_, f);
   ...
   public void tick() {
      ...
      if (!this.noPath()) {
         if (this.canNavigate()) {
            ...
            Vec3d vec3d = this.getEntityPosition();
            Vec3d vec3d1 = this.currentPath.getVectorFromIndex(this.field_75515_a, this.currentPath.getCurrentPathIndex());
            if (vec3d.y > vec3d1.y && !this.field_75515_a.onGround && MathHelper.floor(vec3d.x) == MathHelper.floor(vec3d1.x) && MathHelper.floor(vec3d.z) == MathHelper.floor(vec3d1.z)) {
         ...
         if (!this.noPath()) {
            Vec3d vec3d2 = this.currentPath.getPosition(this.field_75515_a);
            BlockPos blockpos = new BlockPos(vec3d2);
            this.field_75515_a.getMoveHelper().setMoveTo(vec3d2.x, this.world.getBlockState(blockpos.down()).isAir() ? vec3d2.y : WalkNodeProcessor.getGroundY(this.world, blockpos), vec3d2.z, this.speed);
         }
      }
   }
   ...
   protected void pathFollow() {
      Vec3d vec3d = this.getEntityPosition();
      this.maxDistanceToWaypoint = this.field_75515_a.getWidth() > 0.75F ? this.field_75515_a.getWidth() / 2.0F : 0.75F - this.field_75515_a.getWidth() / 2.0F;
      Vec3d vec3d1 = this.currentPath.getCurrentPos();
      if (Math.abs(this.field_75515_a.posX - (vec3d1.x + 0.5D)) < (double)this.maxDistanceToWaypoint && Math.abs(this.field_75515_a.posZ - (vec3d1.z + 0.5D)) < (double)this.maxDistanceToWaypoint && Math.abs(this.field_75515_a.posY - vec3d1.y) < 1.0D) {
         this.currentPath.setCurrentPathIndex(this.currentPath.getCurrentPathIndex() + 1);
      }
   ...
   protected void checkForStuck(Vec3d positionVec3) {
      ...
      if (this.currentPath != null && !this.currentPath.isFinished()) {
         ...
         if (vec3d.equals(this.timeoutCachedNode)) {
            ...
            this.timeoutLimit = this.field_75515_a.getAIMoveSpeed() > 0.0F ? d0 / (double)this.field_75515_a.getAIMoveSpeed() * 1000.0D : 0.0D;
         }
   ...
   protected boolean isInLiquid() {
      return this.field_75515_a.isInWaterOrBubbleColumn() || this.field_75515_a.isInLava();
   }
   ...
   public void func_220970_c(BlockPos p_220970_1_) {
      if (this.currentPath != null && !this.currentPath.isFinished() && this.currentPath.getCurrentPathLength() != 0) {
         ...
         Vec3d vec3d = new Vec3d(((double)pathpoint.x + this.field_75515_a.posX) / 2.0D, ((double)pathpoint.y + this.field_75515_a.posY) / 2.0D, ((double)pathpoint.z + this.field_75515_a.posZ) / 2.0D);
         if (p_220970_1_.func_218137_a(vec3d, (double)(this.currentPath.getCurrentPathLength() - this.currentPath.getCurrentPathIndex()))) {
            this.updatePath();
         }
