...
public class SwimNodeProcessor extends NodeProcessor {
   ...
   public PathPoint getStart() {
      return super.openPoint(MathHelper.floor(this.field_186326_b.getBoundingBox().minX), MathHelper.floor(this.field_186326_b.getBoundingBox().minY + 0.5D), MathHelper.floor(this.field_186326_b.getBoundingBox().minZ));
   }
   ...
   public PathPoint getPathPointToCoords(double x, double y, double z) {
      return super.openPoint(MathHelper.floor(x - (double)(this.field_186326_b.getWidth() / 2.0F)), MathHelper.floor(y + 0.5D), MathHelper.floor(z - (double)(this.field_186326_b.getWidth() / 2.0F)));
   }
   ...
   protected PathPoint openPoint(int x, int y, int z) {
      ...
      PathNodeType pathnodetype = this.getPathNodeType(this.field_186326_b.world, x, y, z);
      float f = this.field_186326_b.getPathPriority(pathnodetype);
      if (f >= 0.0F) {
         pathpoint = super.openPoint(x, y, z);
         pathpoint.nodeType = pathnodetype;
         pathpoint.costMalus = Math.max(pathpoint.costMalus, f);
         if (this.field_176169_a.getFluidState(new BlockPos(x, y, z)).isEmpty()) {
   ...
   private PathNodeType isFree(int p_186327_1_, int p_186327_2_, int p_186327_3_) {
      ...
      for(int i = p_186327_1_; i < p_186327_1_ + this.entitySizeX; ++i) {
         for(int j = p_186327_2_; j < p_186327_2_ + this.entitySizeY; ++j) {
            for(int k = p_186327_3_; k < p_186327_3_ + this.entitySizeZ; ++k) {
               IFluidState ifluidstate = this.field_176169_a.getFluidState(blockpos$mutableblockpos.setPos(i, j, k));
               BlockState blockstate = this.field_176169_a.getBlockState(blockpos$mutableblockpos.setPos(i, j, k));
               if (ifluidstate.isEmpty() && blockstate.allowsMovement(this.field_176169_a, blockpos$mutableblockpos.down(), PathType.WATER) && blockstate.isAir()) {
                  return PathNodeType.BREACH;
               }
      ...
      BlockState blockstate1 = this.field_176169_a.getBlockState(blockpos$mutableblockpos);
      if (blockstate1.allowsMovement(this.field_176169_a, blockpos$mutableblockpos, PathType.WATER)) {
