...
public class WalkNodeProcessor extends NodeProcessor {
   ...
   public PathPoint getStart() {
      ...
      if (this.getCanSwim() && this.field_186326_b.isInWater()) {
         i = MathHelper.floor(this.field_186326_b.getBoundingBox().minY);
         BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos(this.field_186326_b.posX, (double)i, this.field_186326_b.posZ);
         ...
         for(Block block = this.field_176169_a.getBlockState(blockpos$mutableblockpos).getBlock(); block == Blocks.WATER; block = this.field_176169_a.getBlockState(blockpos$mutableblockpos).getBlock()) {
            ++i;
            blockpos$mutableblockpos.setPos(this.field_186326_b.posX, (double)i, this.field_186326_b.posZ);
         }

         --i;
      } else if (this.field_186326_b.onGround) {
         i = MathHelper.floor(this.field_186326_b.getBoundingBox().minY + 0.5D);
      } else {
         BlockPos blockpos;
         for(blockpos = new BlockPos(this.field_186326_b); (this.field_176169_a.getBlockState(blockpos).isAir() || this.field_176169_a.getBlockState(blockpos).allowsMovement(this.field_176169_a, blockpos, PathType.LAND)) && blockpos.getY() > 0; blockpos = blockpos.down()) {
            ;
         }
      ...
      PathNodeType pathnodetype1 = this.getPathNodeType(this.field_186326_b, blockpos2.getX(), i, blockpos2.getZ());
      if (this.field_186326_b.getPathPriority(pathnodetype1) < 0.0F) {
         Set<BlockPos> set = Sets.newHashSet();
         set.add(new BlockPos(this.field_186326_b.getBoundingBox().minX, (double)i, this.field_186326_b.getBoundingBox().minZ));
         set.add(new BlockPos(this.field_186326_b.getBoundingBox().minX, (double)i, this.field_186326_b.getBoundingBox().maxZ));
         set.add(new BlockPos(this.field_186326_b.getBoundingBox().maxX, (double)i, this.field_186326_b.getBoundingBox().minZ));
         set.add(new BlockPos(this.field_186326_b.getBoundingBox().maxX, (double)i, this.field_186326_b.getBoundingBox().maxZ));
   ...
   public int func_222859_a(PathPoint[] p_222859_1_, PathPoint p_222859_2_) {
      int i = 0;
      int j = 0;
      PathNodeType pathnodetype = this.getPathNodeType(this.field_186326_b, p_222859_2_.x, p_222859_2_.y + 1, p_222859_2_.z);
      if (this.field_186326_b.getPathPriority(pathnodetype) >= 0.0F) {
         j = MathHelper.floor(Math.max(1.0F, this.field_186326_b.stepHeight));
      }

      double d0 = getGroundY(this.field_176169_a, new BlockPos(p_222859_2_.x, p_222859_2_.y, p_222859_2_.z));
   ...
   private PathPoint getSafePoint(int x, int y, int z, int stepHeight, double groundYIn, Direction facing) {
      ...
      if (d0 - groundYIn > 1.125D) {
         ...
         double d1 = (double)this.field_186326_b.getWidth() / 2.0D;
         if (f >= 0.0F) {
         ...
         if (pathnodetype == PathNodeType.WALKABLE) {
            ...
            if ((pathpoint == null || pathpoint.costMalus < 0.0F) && stepHeight > 0 && pathnodetype != PathNodeType.FENCE && pathnodetype != PathNodeType.TRAPDOOR) {
               ...
               if (pathpoint != null && (pathpoint.nodeType == PathNodeType.OPEN || pathpoint.nodeType == PathNodeType.WALKABLE) && this.field_186326_b.getWidth() < 1.0F) {
                  double d2 = (double)(x - facing.getXOffset()) + 0.5D;
                  double d3 = (double)(z - facing.getZOffset()) + 0.5D;
                  AxisAlignedBB axisalignedbb = new AxisAlignedBB(d2 - d1, getGroundY(this.field_176169_a, new BlockPos(d2, (double)(y + 1), d3)) + 0.001D, d3 - d1, d2 + d1, (double)this.field_186326_b.getHeight() + getGroundY(this.field_176169_a, new BlockPos(pathpoint.x, pathpoint.y, pathpoint.z)) - 0.002D, d3 + d1);
            ...
            if (pathnodetype == PathNodeType.WATER && !this.getCanSwim()) {
               ...
               while(y > 0) {
                  ...
                  pathpoint = this.openPoint(x, y, z);
                  pathpoint.nodeType = pathnodetype;
                  pathpoint.costMalus = Math.max(pathpoint.costMalus, this.field_186326_b.getPathPriority(pathnodetype));
               }
            }

            if (pathnodetype == PathNodeType.OPEN) {
               AxisAlignedBB axisalignedbb1 = new AxisAlignedBB((double)x - d1 + 0.5D, (double)y + 0.001D, (double)z - d1 + 0.5D, (double)x + d1 + 0.5D, (double)((float)y + this.field_186326_b.getHeight()), (double)z + d1 + 0.5D);
               if (!this.field_176169_a.isCollisionBoxesEmpty(this.field_186326_b, axisalignedbb1)) {
                  return null;
               }

               if (this.field_186326_b.getWidth() >= 1.0F) {
   ...
   private PathNodeType getPathNodeType(MobEntity entitylivingIn, int x, int y, int z) {
      return this.getPathNodeType(this.field_176169_a, x, y, z, entitylivingIn, this.entitySizeX, this.entitySizeY, this.entitySizeZ, this.getCanOpenDoors(), this.getCanEnterDoors());
   }
