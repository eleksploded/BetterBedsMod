...
public class MobEffectsPredicate {
   ...
   public static MobEffectsPredicate deserialize(@Nullable JsonElement element) {
      if (element != null && !element.isJsonNull()) {
         ...
         for(Entry<String, JsonElement> entry : jsonobject.entrySet()) {
            ...
            Effect effect = Registry.field_212631_t.func_218349_b(resourcelocation).orElseThrow(() -> {
   ...
   public JsonElement serialize() {
      if (this == ANY) {
         ...
         for(Entry<Effect, MobEffectsPredicate.InstancePredicate> entry : this.effects.entrySet()) {
            jsonobject.add(Registry.field_212631_t.getKey(entry.getKey()).toString(), entry.getValue().serialize());
         }
