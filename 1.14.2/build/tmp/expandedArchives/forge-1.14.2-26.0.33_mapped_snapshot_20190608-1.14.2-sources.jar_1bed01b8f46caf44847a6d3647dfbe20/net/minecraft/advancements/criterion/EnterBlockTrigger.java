...
public class EnterBlockTrigger implements ICriterionTrigger<EnterBlockTrigger.Instance> {
   ...
   public EnterBlockTrigger.Instance deserializeInstance(JsonObject json, JsonDeserializationContext context) {
      ...
      if (json.has("block")) {
         ...
         block = Registry.field_212618_g.func_218349_b(resourcelocation).orElseThrow(() -> {
      ...
      if (json.has("state")) {
         ...
         for(Entry<String, JsonElement> entry : JSONUtils.getJsonObject(json, "state").entrySet()) {
            ...
            if (iproperty == null) {
               throw new JsonSyntaxException("Unknown block state property '" + (String)entry.getKey() + "' for block '" + Registry.field_212618_g.getKey(block) + "'");
            }

            String s = JSONUtils.getString(entry.getValue(), entry.getKey());
            Optional<?> optional = iproperty.parseValue(s);
            if (!optional.isPresent()) {
               throw new JsonSyntaxException("Invalid block state value '" + s + "' for property '" + (String)entry.getKey() + "' on block '" + Registry.field_212618_g.getKey(block) + "'");
            }
   ...
   public static class Instance extends CriterionInstance {
      ...
      public JsonElement serialize() {
         ...
         if (this.block != null) {
            jsonobject.addProperty("block", Registry.field_212618_g.getKey(this.block).toString());
