...
public class AdvancementRewards {
   ...
   public void apply(ServerPlayerEntity player) {
      ...
      LootContext lootcontext = (new LootContext.Builder(player.getServerWorld())).withParameter(LootParameters.field_216281_a, player).withParameter(LootParameters.field_216286_f, new BlockPos(player)).withRandom(player.getRNG()).withLuck(player.getLuck()).build(LootParameterSets.field_216265_f); // FORGE: luck to LootContext
      boolean flag = false;

      for(ResourceLocation resourcelocation : this.loot) {
         for(ItemStack itemstack : player.server.getLootTableManager().getLootTableFromLocation(resourcelocation).func_216113_a(lootcontext)) {
