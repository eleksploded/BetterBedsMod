...
public abstract class FlowingFluid extends Fluid {
   ...
   protected boolean causesDownwardCurrent(IBlockReader worldIn, BlockPos neighborPos, Direction side) {
      ...
      if (ifluidstate.getFluid().isEquivalentTo(this)) {
         ...
         return blockstate.getMaterial() == Material.ICE ? false : Block.func_220056_d(blockstate, worldIn, neighborPos, side);
      }
   }
   ...
   private boolean isBlocked(IBlockReader worldIn, BlockPos pos, BlockState state, Fluid fluidIn) {
      ...
      } else if (!(block instanceof DoorBlock) && !block.isIn(BlockTags.field_219753_V) && block != Blocks.LADDER && block != Blocks.SUGAR_CANE && block != Blocks.BUBBLE_COLUMN) {
