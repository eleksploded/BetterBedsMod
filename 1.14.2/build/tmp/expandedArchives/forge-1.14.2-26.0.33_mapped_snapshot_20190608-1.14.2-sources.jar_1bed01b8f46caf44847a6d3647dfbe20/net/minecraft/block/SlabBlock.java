...
public class SlabBlock extends Block implements IWaterLoggable {
   ...
   public VoxelShape getShape(BlockState p_220053_1_, IBlockReader p_220053_2_, BlockPos p_220053_3_, ISelectionContext p_220053_4_) {
      SlabType slabtype = p_220053_1_.get(TYPE);
      switch(slabtype) {
      case DOUBLE:
   ...
   public BlockState getStateForPlacement(BlockItemUseContext context) {
      ...
      if (blockstate.getBlock() == this) {
         ...
         return direction != Direction.DOWN && (direction == Direction.UP || !(context.func_221532_j().y - (double)blockpos.getY() > 0.5D)) ? blockstate1 : blockstate1.with(TYPE, SlabType.TOP);
      }
   }
   ...
   public boolean isReplaceable(BlockState state, BlockItemUseContext useContext) {
      ...
      if (slabtype != SlabType.DOUBLE && itemstack.getItem() == this.asItem()) {
         if (useContext.replacingClickedOnBlock()) {
            boolean flag = useContext.func_221532_j().y - (double)useContext.getPos().getY() > 0.5D;
