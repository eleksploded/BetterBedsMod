...
public class NetherPortalBlock extends Block {
   ...
   public void tick(BlockState state, World worldIn, BlockPos pos, Random random) {
      if (worldIn.dimension.isSurfaceWorld() && worldIn.getGameRules().getBoolean("doMobSpawning") && random.nextInt(2000) < worldIn.getDifficulty().getId()) {
         ...
         if (worldIn.getBlockState(pos).canEntitySpawn(worldIn, pos, EntityType.ZOMBIE_PIGMAN)) {
            Entity entity = EntityType.ZOMBIE_PIGMAN.func_220342_a(worldIn, (CompoundNBT)null, (ITextComponent)null, (PlayerEntity)null, pos.up(), SpawnReason.STRUCTURE, false, false);
            if (entity != null) {
   ...
   public BlockPattern.PatternHelper createPatternHelper(IWorld worldIn, BlockPos p_181089_2_) {
      ...
      if (!netherportalblock$size.isValid()) {
         ...
         Direction direction = netherportalblock$size.field_150866_c.rotateYCCW();
         ...
         for(Direction.AxisDirection direction$axisdirection : Direction.AxisDirection.values()) {
            BlockPattern.PatternHelper blockpattern$patternhelper = new BlockPattern.PatternHelper(direction.getAxisDirection() == direction$axisdirection ? blockpos : blockpos.offset(netherportalblock$size.field_150866_c, netherportalblock$size.getWidth() - 1), Direction.getFacingFromAxis(direction$axisdirection, direction$axis), Direction.UP, loadingcache, netherportalblock$size.getWidth(), netherportalblock$size.getHeight(), 1);
         ...
         return new BlockPattern.PatternHelper(direction.getAxisDirection() == direction$axisdirection1 ? blockpos : blockpos.offset(netherportalblock$size.field_150866_c, netherportalblock$size.getWidth() - 1), Direction.getFacingFromAxis(direction$axisdirection1, direction$axis), Direction.UP, loadingcache, netherportalblock$size.getWidth(), netherportalblock$size.getHeight(), 1);
      }
   }
   ...
   public static class Size {
      ...
      public Size(IWorld p_i48740_1_, BlockPos p_i48740_2_, Direction.Axis p_i48740_3_) {
         ...
         int i = this.getDistanceUntilEdge(p_i48740_2_, this.field_150863_d) - 1;
         if (i >= 0) {
            this.bottomLeft = p_i48740_2_.offset(this.field_150863_d, i);
            this.width = this.getDistanceUntilEdge(this.bottomLeft, this.field_150866_c);
      ...
      protected int calculatePortalHeight() {
         ...
         for(this.height = 0; this.height < 21; ++this.height) {
            for(int i = 0; i < this.width; ++i) {
               BlockPos blockpos = this.bottomLeft.offset(this.field_150866_c, i).up(this.height);
         ...
         for(int j = 0; j < this.width; ++j) {
            if (this.world.getBlockState(this.bottomLeft.offset(this.field_150866_c, j).up(this.height)).getBlock() != Blocks.OBSIDIAN) {
               this.height = 0;
               break;
            }
         }
      ...
      public void placePortalBlocks() {
         for(int i = 0; i < this.width; ++i) {
            BlockPos blockpos = this.bottomLeft.offset(this.field_150866_c, i);

            for(int j = 0; j < this.height; ++j) {
               this.world.setBlockState(blockpos.up(j), Blocks.NETHER_PORTAL.getDefaultState().with(NetherPortalBlock.AXIS, this.field_150865_b), 18);
            }
         }
