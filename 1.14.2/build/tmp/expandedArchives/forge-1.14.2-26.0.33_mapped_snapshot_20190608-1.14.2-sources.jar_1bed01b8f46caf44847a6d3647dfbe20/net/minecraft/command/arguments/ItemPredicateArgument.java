...
public class ItemPredicateArgument implements ArgumentType<ItemPredicateArgument.IResult> {
   ...
   static class ItemPredicate implements Predicate<ItemStack> {
      ...
      @Nullable
      private final CompoundNBT field_199842_b;

      public ItemPredicate(Item itemIn, @Nullable CompoundNBT nbtIn) {
         this.item = itemIn;
         this.field_199842_b = nbtIn;
      }

      public boolean test(ItemStack p_test_1_) {
         return p_test_1_.getItem() == this.item && NBTUtil.areNBTEquals(this.field_199842_b, p_test_1_.getTag(), true);
      }
   }
   ...
   static class TagPredicate implements Predicate<ItemStack> {
      ...
      @Nullable
      private final CompoundNBT field_199844_b;

      public TagPredicate(Tag<Item> tagIn, @Nullable CompoundNBT nbtIn) {
         this.tag = tagIn;
         this.field_199844_b = nbtIn;
      }

      public boolean test(ItemStack p_test_1_) {
         return this.tag.contains(p_test_1_.getItem()) && NBTUtil.areNBTEquals(this.field_199844_b, p_test_1_.getTag(), true);
      }
   }
}