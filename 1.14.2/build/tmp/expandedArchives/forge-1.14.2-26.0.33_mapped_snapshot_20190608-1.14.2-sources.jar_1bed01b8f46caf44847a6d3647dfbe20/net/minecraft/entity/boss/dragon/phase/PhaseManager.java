...
public class PhaseManager {
   ...
   public void setPhase(PhaseType<?> phaseIn) {
      if (this.phase == null || phaseIn != this.phase.getType()) {
         ...
         if (!this.field_188760_b.world.isRemote) {
            this.field_188760_b.getDataManager().set(EnderDragonEntity.PHASE, phaseIn.getId());
         }

         LOGGER.debug("Dragon is now in phase {} on the {}", phaseIn, this.field_188760_b.world.isRemote ? "client" : "server");
   ...
   public <T extends IPhase> T getPhase(PhaseType<T> phaseIn) {
      ...
      if (this.phases[i] == null) {
         this.phases[i] = phaseIn.createPhase(this.field_188760_b);
      }
