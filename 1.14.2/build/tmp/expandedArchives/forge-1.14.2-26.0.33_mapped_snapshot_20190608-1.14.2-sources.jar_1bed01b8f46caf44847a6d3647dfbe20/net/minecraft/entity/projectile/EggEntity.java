...
public class EggEntity extends ProjectileItemEntity {
   ...
   public void handleStatusUpdate(byte id) {
      if (id == 3) {
         ...
         for(int i = 0; i < 8; ++i) {
            this.world.addParticle(new ItemParticleData(ParticleTypes.ITEM, this.getPotion()), this.posX, this.posY, this.posZ, ((double)this.rand.nextFloat() - 0.5D) * 0.08D, ((double)this.rand.nextFloat() - 0.5D) * 0.08D, ((double)this.rand.nextFloat() - 0.5D) * 0.08D);
         }
      }
      ...
      if (!this.world.isRemote) {
         if (this.rand.nextInt(8) == 0) {
            ...
            for(int j = 0; j < i; ++j) {
               ...
               this.world.func_217376_c(chickenentity);
            }
         }
