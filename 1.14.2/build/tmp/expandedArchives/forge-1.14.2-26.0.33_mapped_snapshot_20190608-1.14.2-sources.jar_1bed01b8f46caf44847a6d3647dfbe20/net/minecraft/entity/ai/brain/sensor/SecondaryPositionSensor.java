...
public class SecondaryPositionSensor extends Sensor<VillagerEntity> {
   ...
   protected void update(ServerWorld p_212872_1_, VillagerEntity p_212872_2_) {
      ...
      for(int j = -4; j <= 4; ++j) {
         for(int k = -2; k <= 2; ++k) {
            for(int l = -4; l <= 4; ++l) {
               ...
               if (p_212872_2_.func_213700_eh().getProfession().func_221150_d().contains(p_212872_1_.getBlockState(blockpos1).getBlock())) {
      ...
      if (!list.isEmpty()) {
         brain.setMemory(MemoryModuleType.field_220944_e, list);
      } else {
         brain.removeMemory(MemoryModuleType.field_220944_e);
      }
