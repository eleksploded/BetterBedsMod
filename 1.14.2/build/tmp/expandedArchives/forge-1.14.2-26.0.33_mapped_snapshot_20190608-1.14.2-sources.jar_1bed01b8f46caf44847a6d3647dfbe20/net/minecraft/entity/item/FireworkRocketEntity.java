...
public class FireworkRocketEntity extends Entity implements IRendersAsItem, IProjectile {
   private static final DataParameter<ItemStack> FIREWORK_ITEM = EntityDataManager.createKey(FireworkRocketEntity.class, DataSerializers.field_187196_f);
   private static final DataParameter<OptionalInt> BOOSTED_ENTITY_ID = EntityDataManager.createKey(FireworkRocketEntity.class, DataSerializers.field_218814_r);
   private static final DataParameter<Boolean> field_213895_d = EntityDataManager.createKey(FireworkRocketEntity.class, DataSerializers.field_187198_h);
   ...
   public void tick() {
      ...
      if (this.isAttachedToEntity()) {
         ...
         if (this.field_191513_e != null) {
            if (this.field_191513_e.isElytraFlying()) {
               ...
               double d0 = 1.5D;
               double d1 = 0.1D;
               Vec3d vec3d1 = this.field_191513_e.getMotion();
               this.field_191513_e.setMotion(vec3d1.add(vec3d.x * 0.1D + (vec3d.x * 1.5D - vec3d1.x) * 0.5D, vec3d.y * 0.1D + (vec3d.y * 1.5D - vec3d1.y) * 0.5D, vec3d.z * 0.1D + (vec3d.z * 1.5D - vec3d1.z) * 0.5D));
            }

            this.setPosition(this.field_191513_e.posX, this.field_191513_e.posY, this.field_191513_e.posZ);
            this.setMotion(this.field_191513_e.getMotion());
         }
      } else {
      ...
      RayTraceResult raytraceresult = ProjectileHelper.func_221267_a(this, this.getBoundingBox().func_216361_a(vec3d2).grow(1.0D), (p_213890_0_) -> {
   ...
   private void dealExplosionDamage() {
      ...
      if (f > 0.0F) {
         if (this.field_191513_e != null) {
            this.field_191513_e.attackEntityFrom(DamageSource.FIREWORKS, 5.0F + (float)(listnbt.size() * 2));
         }
         ...
         for(LivingEntity livingentity : this.world.getEntitiesWithinAABB(LivingEntity.class, this.getBoundingBox().grow(5.0D))) {
            if (livingentity != this.field_191513_e && !(this.getDistanceSq(livingentity) > 25.0D)) {
               boolean flag = false;

               for(int i = 0; i < 2; ++i) {
                  Vec3d vec3d1 = new Vec3d(livingentity.posX, livingentity.posY + (double)livingentity.getHeight() * 0.5D * (double)i, livingentity.posZ);
                  RayTraceResult raytraceresult = this.world.func_217299_a(new RayTraceContext(vec3d, vec3d1, RayTraceContext.BlockMode.COLLIDER, RayTraceContext.FluidMode.NONE, this));
   ...
   public void writeAdditional(CompoundNBT p_213281_1_) {
      ...
      if (!itemstack.isEmpty()) {
         p_213281_1_.put("FireworksItem", itemstack.write(new CompoundNBT()));
      }

      p_213281_1_.putBoolean("ShotAtAngle", this.dataManager.get(field_213895_d));
   }
