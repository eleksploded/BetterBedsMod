...
public class SquidRenderer extends MobRenderer<SquidEntity, SquidModel<SquidEntity>> {
   ...
   protected void applyRotations(SquidEntity entityLiving, float ageInTicks, float rotationYaw, float partialTicks) {
      float f = MathHelper.func_219799_g(partialTicks, entityLiving.prevSquidPitch, entityLiving.squidPitch);
      float f1 = MathHelper.func_219799_g(partialTicks, entityLiving.prevSquidYaw, entityLiving.squidYaw);
   ...
   protected float handleRotationFloat(SquidEntity livingBase, float partialTicks) {
      return MathHelper.func_219799_g(partialTicks, livingBase.lastTentacleAngle, livingBase.tentacleAngle);
   }
}