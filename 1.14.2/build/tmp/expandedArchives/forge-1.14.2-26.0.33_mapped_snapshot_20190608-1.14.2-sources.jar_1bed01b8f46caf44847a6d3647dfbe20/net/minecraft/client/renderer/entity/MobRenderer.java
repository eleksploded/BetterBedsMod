...
public abstract class MobRenderer<T extends MobEntity, M extends EntityModel<T>> extends LivingRenderer<T, M> {
   ...
   protected void renderLeash(T entityLivingIn, double x, double y, double z, float entityYaw, float partialTicks) {
      ...
      if (entity != null) {
         ...
         double d0 = (double)(MathHelper.func_219799_g(partialTicks * 0.5F, entity.rotationYaw, entity.prevRotationYaw) * ((float)Math.PI / 180F));
         double d1 = (double)(MathHelper.func_219799_g(partialTicks * 0.5F, entity.rotationPitch, entity.prevRotationPitch) * ((float)Math.PI / 180F));
         ...
         double d6 = MathHelper.func_219803_d((double)partialTicks, entity.prevPosX, entity.posX) - d2 * 0.7D - d3 * 0.5D * d5;
         double d7 = MathHelper.func_219803_d((double)partialTicks, entity.prevPosY + (double)entity.getEyeHeight() * 0.7D, entity.posY + (double)entity.getEyeHeight() * 0.7D) - d4 * 0.5D - 0.25D;
         double d8 = MathHelper.func_219803_d((double)partialTicks, entity.prevPosZ, entity.posZ) - d3 * 0.7D + d2 * 0.5D * d5;
         double d9 = (double)(MathHelper.func_219799_g(partialTicks, entityLivingIn.renderYawOffset, entityLivingIn.prevRenderYawOffset) * ((float)Math.PI / 180F)) + (Math.PI / 2D);
         d2 = Math.cos(d9) * (double)entityLivingIn.getWidth() * 0.4D;
         d3 = Math.sin(d9) * (double)entityLivingIn.getWidth() * 0.4D;
         double d10 = MathHelper.func_219803_d((double)partialTicks, entityLivingIn.prevPosX, entityLivingIn.posX) + d2;
         double d11 = MathHelper.func_219803_d((double)partialTicks, entityLivingIn.prevPosY, entityLivingIn.posY);
         double d12 = MathHelper.func_219803_d((double)partialTicks, entityLivingIn.prevPosZ, entityLivingIn.posZ) + d3;
         x = x + d2;
         z = z + d3;
