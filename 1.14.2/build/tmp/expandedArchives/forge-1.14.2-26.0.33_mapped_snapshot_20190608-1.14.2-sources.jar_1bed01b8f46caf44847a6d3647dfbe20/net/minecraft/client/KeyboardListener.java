...
public class KeyboardListener {
   ...
   private void printDebugMessage(String message, Object... args) {
      this.mc.field_71456_v.getChatGUI().printChatMessage((new StringTextComponent("")).appendSibling((new TranslationTextComponent("debug.prefix")).applyTextStyles(new TextFormatting[]{TextFormatting.YELLOW, TextFormatting.BOLD})).appendText(" ").appendSibling(new TranslationTextComponent(message, args)));
   }

   private void printDebugWarning(String message, Object... args) {
      this.mc.field_71456_v.getChatGUI().printChatMessage((new StringTextComponent("")).appendSibling((new TranslationTextComponent("debug.prefix")).applyTextStyles(new TextFormatting[]{TextFormatting.RED, TextFormatting.BOLD})).appendText(" ").appendSibling(new TranslationTextComponent(message, args)));
   }
      ...
      if (this.debugCrashKeyPressTime > 0L && this.debugCrashKeyPressTime < Util.milliTime() - 100L) {
         ...
         case 68:
            if (this.mc.field_71456_v != null) {
               this.mc.field_71456_v.getChatGUI().clearChatMessages(false);
            }
         ...
         default:
            return false;
         case 70:
            AbstractOption.field_216706_m.func_216727_a(this.mc.gameSettings, MathHelper.clamp((double)(this.mc.gameSettings.renderDistanceChunks + (Screen.hasShiftDown() ? -1 : 1)), AbstractOption.field_216706_m.func_216732_b(), AbstractOption.field_216706_m.func_216733_c()));
         ...
         case 81:
            ...
            NewChatGui newchatgui = this.mc.field_71456_v.getChatGUI();
   ...
   private void copyHoveredObject(boolean privileged, boolean askServer) {
      ...
      if (raytraceresult != null) {
         ...
         case BLOCK:
            ...
            if (privileged) {
               if (askServer) {
                  this.mc.player.field_71174_a.getNBTQueryManager().queryTileEntity(blockpos, (p_211561_3_) -> {
         ...
         case ENTITY:
            ...
            ResourceLocation resourcelocation = Registry.field_212629_r.getKey(entity.getType());
            Vec3d vec3d = new Vec3d(entity.posX, entity.posY, entity.posZ);
            if (privileged) {
               if (askServer) {
                  this.mc.player.field_71174_a.getNBTQueryManager().queryEntity(entity.getEntityId(), (p_211560_3_) -> {
   ...
   public void onKeyEvent(long windowPointer, int key, int scanCode, int action, int modifiers) {
      if (windowPointer == this.mc.mainWindow.getHandle()) {
         if (this.debugCrashKeyPressTime > 0L) {
            if (!InputMappings.func_216506_a(Minecraft.getInstance().mainWindow.getHandle(), 67) || !InputMappings.func_216506_a(Minecraft.getInstance().mainWindow.getHandle(), 292)) {
               this.debugCrashKeyPressTime = -1L;
            }
         } else if (InputMappings.func_216506_a(Minecraft.getInstance().mainWindow.getHandle(), 67) && InputMappings.func_216506_a(Minecraft.getInstance().mainWindow.getHandle(), 292)) {
         ...
         INestedGuiEventHandler inestedguieventhandler = this.mc.field_71462_r;

         if ((!(this.mc.field_71462_r instanceof ControlsScreen) || ((ControlsScreen)inestedguieventhandler).time <= Util.milliTime() - 20L)) {
            if (action == 1) {
            ...
            if (this.mc.gameSettings.keyBindScreenshot.matchesKey(key, scanCode)) {
               ...
               ScreenShotHelper.saveScreenshot(this.mc.gameDir, this.mc.mainWindow.getFramebufferWidth(), this.mc.mainWindow.getFramebufferHeight(), this.mc.getFramebuffer(), (p_212449_1_) -> {
                  this.mc.execute(() -> {
                     this.mc.field_71456_v.getChatGUI().printChatMessage(p_212449_1_);
                  });
               });
               return;
            }
            } else if (action == 0 /*GLFW_RELEASE*/ && this.mc.field_71462_r instanceof ControlsScreen)
               ((ControlsScreen)this.mc.field_71462_r).buttonId = null; //Forge: Unset pure modifiers.
         }
         ...
         if (action != 0 && key == 66 && Screen.hasControlDown() && flag) {
            AbstractOption.field_216715_v.func_216722_a(this.mc.gameSettings, 1);
         ...
         if (inestedguieventhandler != null) {
            ...
            Screen.wrapScreenError(() -> {
               if (action != 1 && (action != 2 || !this.repeatEventsEnabled)) {
                  if (action == 0) {
                     aboolean[0] = net.minecraftforge.client.ForgeHooksClient.onGuiKeyReleasedPre(this.mc.field_71462_r, key, scanCode, modifiers);
                     if (!aboolean[0]) aboolean[0] = inestedguieventhandler.keyReleased(key, scanCode, modifiers);
                     if (!aboolean[0]) aboolean[0] = net.minecraftforge.client.ForgeHooksClient.onGuiKeyReleasedPost(this.mc.field_71462_r, key, scanCode, modifiers);
                  }
               } else {
                  aboolean[0] = net.minecraftforge.client.ForgeHooksClient.onGuiKeyPressedPre(this.mc.field_71462_r, key, scanCode, modifiers);
                  if (!aboolean[0]) aboolean[0] = inestedguieventhandler.keyPressed(key, scanCode, modifiers);
                  if (!aboolean[0]) aboolean[0] = net.minecraftforge.client.ForgeHooksClient.onGuiKeyPressedPost(this.mc.field_71462_r, key, scanCode, modifiers);
               }
         ...
         if (this.mc.field_71462_r == null || this.mc.field_71462_r.passEvents) {
            ...
            if (action == 0) {
               ...
               if (this.mc.field_71462_r == null) {
                  if (key == 256) {
                     boolean flag2 = InputMappings.func_216506_a(Minecraft.getInstance().mainWindow.getHandle(), 292);
                     this.mc.displayInGameMenu(flag2);
                  }

                  flag1 = InputMappings.func_216506_a(Minecraft.getInstance().mainWindow.getHandle(), 292) && this.processKeyF3(key);
   ...
   private void onCharEvent(long windowPointer, int codePoint, int modifiers) {
      if (windowPointer == this.mc.mainWindow.getHandle()) {
         IGuiEventListener iguieventlistener = this.mc.field_71462_r;
         if (iguieventlistener != null && this.mc.func_213250_au() == null) {
            if (Character.charCount(codePoint) == 1) {
               Screen.wrapScreenError(() -> {
                  if (net.minecraftforge.client.ForgeHooksClient.onGuiCharTypedPre(this.mc.field_71462_r, (char)codePoint, modifiers)) return;
                  if (iguieventlistener.charTyped((char)codePoint, modifiers)) return;
                  net.minecraftforge.client.ForgeHooksClient.onGuiCharTypedPost(this.mc.field_71462_r, (char)codePoint, modifiers);
               }, "charTyped event handler", iguieventlistener.getClass().getCanonicalName());
            } else {
               for(char c0 : Character.toChars(codePoint)) {
                  Screen.wrapScreenError(() -> {
                     if (net.minecraftforge.client.ForgeHooksClient.onGuiCharTypedPre(this.mc.field_71462_r, c0, modifiers)) return;
                     if (iguieventlistener.charTyped(c0, modifiers)) return;
                     net.minecraftforge.client.ForgeHooksClient.onGuiCharTypedPost(this.mc.field_71462_r, c0, modifiers);
                  }, "charTyped event handler", iguieventlistener.getClass().getCanonicalName());
               }
            }
