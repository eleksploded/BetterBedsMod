...
public class SilverfishModel<T extends Entity> extends EntityModel<T> {
   ...
   public void func_212844_a_(T p_212844_1_, float p_212844_2_, float p_212844_3_, float p_212844_4_, float p_212844_5_, float p_212844_6_, float p_212844_7_) {
      for(int i = 0; i < this.field_78171_a.length; ++i) {
         this.field_78171_a[i].rotateAngleY = MathHelper.cos(p_212844_4_ * 0.9F + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * 0.05F * (float)(1 + Math.abs(i - 2));
         this.field_78171_a[i].rotationPointX = MathHelper.sin(p_212844_4_ * 0.9F + (float)i * 0.15F * (float)Math.PI) * (float)Math.PI * 0.2F * (float)Math.abs(i - 2);
      }
