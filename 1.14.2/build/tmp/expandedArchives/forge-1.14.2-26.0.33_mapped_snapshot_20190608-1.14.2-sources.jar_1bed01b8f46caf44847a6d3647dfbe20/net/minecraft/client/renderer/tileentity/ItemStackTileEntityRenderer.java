...
public class ItemStackTileEntityRenderer {
   private static final ShulkerBoxTileEntity[] field_191274_b = Arrays.stream(DyeColor.values()).sorted(Comparator.comparingInt(DyeColor::getId)).map(ShulkerBoxTileEntity::new).toArray((p_199929_0_) -> {
   ...
   public void renderByItem(ItemStack itemStackIn) {
      ...
      if (item instanceof BannerItem) {
         this.field_179024_e.loadFromItemStack(itemStackIn, ((BannerItem)item).getColor());
         TileEntityRendererDispatcher.instance.renderAsItem(this.field_179024_e);
      } else if (item instanceof BlockItem && ((BlockItem)item).getBlock() instanceof BedBlock) {
         this.field_193843_g.setColor(((BedBlock)((BlockItem)item).getBlock()).getColor());
         TileEntityRendererDispatcher.instance.renderAsItem(this.field_193843_g);
      } else if (item == Items.SHIELD) {
         if (itemStackIn.getChildTag("BlockEntityTag") != null) {
            this.field_179024_e.loadFromItemStack(itemStackIn, ShieldItem.getColor(itemStackIn));
            Minecraft.getInstance().getTextureManager().bindTexture(BannerTextures.SHIELD_DESIGNS.getResourceLocation(this.field_179024_e.getPatternResourceLocation(), this.field_179024_e.getPatternList(), this.field_179024_e.getColorList()));
         } else {
         ...
         this.field_187318_g.render();
         if (itemStackIn.hasEffect()) {
            this.renderEffect(this.field_187318_g::render);
         }
         ...
         if (SkullTileEntityRenderer.field_147536_b != null) {
            ...
            SkullTileEntityRenderer.field_147536_b.render(0.0F, 0.0F, 0.0F, (Direction)null, 180.0F, ((AbstractSkullBlock)((BlockItem)item).getBlock()).getSkullType(), gameprofile, -1, 0.0F);
         ...
         GlStateManager.pushMatrix();
         GlStateManager.scalef(1.0F, -1.0F, -1.0F);
         this.field_203084_j.renderer();
         if (itemStackIn.hasEffect()) {
            this.renderEffect(this.field_203084_j::renderer);
         }

         GlStateManager.popMatrix();
      } else if (item instanceof BlockItem && ((BlockItem)item).getBlock() == Blocks.CONDUIT) {
         TileEntityRendererDispatcher.instance.renderAsItem(this.field_205085_j);
      } else if (item == Blocks.ENDER_CHEST.asItem()) {
         TileEntityRendererDispatcher.instance.renderAsItem(this.field_147716_d);
      } else if (item == Blocks.TRAPPED_CHEST.asItem()) {
         TileEntityRendererDispatcher.instance.renderAsItem(this.field_147718_c);
      } else if (Block.getBlockFromItem(item) instanceof ShulkerBoxBlock) {
         DyeColor dyecolor = ShulkerBoxBlock.getColorFromItem(item);
         if (dyecolor == null) {
            TileEntityRendererDispatcher.instance.renderAsItem(field_204401_c);
         } else {
            TileEntityRendererDispatcher.instance.renderAsItem(field_191274_b[dyecolor.getId()]);
         }
      } else {
         TileEntityRendererDispatcher.instance.renderAsItem(this.field_147717_b);
      }
