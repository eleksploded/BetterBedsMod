...
public class PlayerController {
   ...
   public void setPlayerCapabilities(PlayerEntity player) {
      this.currentGameType.configurePlayerCapabilities(player.playerAbilities);
   }
   ...
   public void setGameType(GameType type) {
      ...
      this.currentGameType.configurePlayerCapabilities(this.mc.player.playerAbilities);
   }
   ...
   public boolean clickBlock(BlockPos loc, Direction face) {
      ...
      if (!this.mc.world.getWorldBorder().contains(loc)) {
         ...
         if (this.currentGameType.isCreative()) {
            ...
            this.field_78774_b.sendPacket(new CPlayerDiggingPacket(CPlayerDiggingPacket.Action.START_DESTROY_BLOCK, loc, face));
            clickBlockCreative(this.mc, this, loc, face);
            this.blockHitDelay = 5;
         } else if (!this.isHittingBlock || !this.isHittingPosition(loc)) {
            if (this.isHittingBlock) {
               this.field_78774_b.sendPacket(new CPlayerDiggingPacket(CPlayerDiggingPacket.Action.ABORT_DESTROY_BLOCK, this.currentBlock, face));
            }
   ...
   public void resetBlockRemoving() {
      if (this.isHittingBlock) {
         ...
         this.field_78774_b.sendPacket(new CPlayerDiggingPacket(CPlayerDiggingPacket.Action.ABORT_DESTROY_BLOCK, this.currentBlock, Direction.DOWN));
   ...
   public boolean onPlayerDamageBlock(BlockPos posBlock, Direction directionFacing) {
      ...
      if (this.blockHitDelay > 0) {
         ...
         this.field_78774_b.sendPacket(new CPlayerDiggingPacket(CPlayerDiggingPacket.Action.START_DESTROY_BLOCK, posBlock, directionFacing));
         ...
         if (blockstate.isAir(this.mc.world, posBlock)) {
            ...
            if (this.curBlockDamageMP >= 1.0F) {
               this.isHittingBlock = false;
               this.field_78774_b.sendPacket(new CPlayerDiggingPacket(CPlayerDiggingPacket.Action.STOP_DESTROY_BLOCK, posBlock, directionFacing));
   ...
   public void tick() {
      this.syncCurrentPlayItem();
      if (this.field_78774_b.getNetworkManager().isChannelOpen()) {
         this.field_78774_b.getNetworkManager().tick();
      } else {
         this.field_78774_b.getNetworkManager().handleDisconnection();
      }
   ...
   private void syncCurrentPlayItem() {
      ...
      if (i != this.currentPlayerItem) {
         ...
         this.field_78774_b.sendPacket(new CHeldItemChangePacket(this.currentPlayerItem));
      }
   ...
   public ActionResultType func_217292_a(ClientPlayerEntity p_217292_1_, ClientWorld p_217292_2_, Hand p_217292_3_, BlockRayTraceResult p_217292_4_) {
      ...
      if (!this.mc.world.getWorldBorder().contains(blockpos)) {
         ...
         if (this.currentGameType == GameType.SPECTATOR) {
            this.field_78774_b.sendPacket(new CPlayerTryUseItemOnBlockPacket(p_217292_3_, p_217292_4_));
   ...
   public ActionResultType processRightClick(PlayerEntity player, World worldIn, Hand hand) {
      if (this.currentGameType == GameType.SPECTATOR) {
         ...
         this.syncCurrentPlayItem();
         this.field_78774_b.sendPacket(new CPlayerTryUseItemPacket(hand));
   ...
   public ClientPlayerEntity createPlayer(ClientWorld p_199681_1_, StatisticsManager p_199681_2_, ClientRecipeBook p_199681_3_) {
      return new ClientPlayerEntity(this.mc, p_199681_1_, this.field_78774_b, p_199681_2_, p_199681_3_);
   }
   ...
   public void attackEntity(PlayerEntity playerIn, Entity targetEntity) {
      this.syncCurrentPlayItem();
      this.field_78774_b.sendPacket(new CUseEntityPacket(targetEntity));
      if (this.currentGameType != GameType.SPECTATOR) {
   ...
   public ActionResultType interactWithEntity(PlayerEntity player, Entity target, Hand hand) {
      this.syncCurrentPlayItem();
      this.field_78774_b.sendPacket(new CUseEntityPacket(target, hand));
   ...
   public ActionResultType interactWithEntity(PlayerEntity player, Entity target, EntityRayTraceResult ray, Hand hand) {
      ...
      this.field_78774_b.sendPacket(new CUseEntityPacket(target, hand, vec3d));
      if (this.currentGameType == GameType.SPECTATOR) return ActionResultType.PASS; // don't fire for spectators to match non-specific EntityInteract
   ...
   public ItemStack windowClick(int windowId, int slotId, int mouseButton, ClickType type, PlayerEntity player) {
      ...
      this.field_78774_b.sendPacket(new CClickWindowPacket(windowId, slotId, mouseButton, type, itemstack, short1));
      return itemstack;
   }

   public void func_203413_a(int p_203413_1_, IRecipe<?> p_203413_2_, boolean p_203413_3_) {
      this.field_78774_b.sendPacket(new CPlaceRecipePacket(p_203413_1_, p_203413_2_, p_203413_3_));
   }
   ...
   public void sendEnchantPacket(int windowID, int button) {
      this.field_78774_b.sendPacket(new CEnchantItemPacket(windowID, button));
   }
   ...
   public void sendSlotPacket(ItemStack itemStackIn, int slotId) {
      if (this.currentGameType.isCreative()) {
         this.field_78774_b.sendPacket(new CCreativeInventoryActionPacket(slotId, itemStackIn));
      }
   ...
   public void sendPacketDropItem(ItemStack itemStackIn) {
      if (this.currentGameType.isCreative() && !itemStackIn.isEmpty()) {
         this.field_78774_b.sendPacket(new CCreativeInventoryActionPacket(-1, itemStackIn));
      }
   ...
   public void onStoppedUsingItem(PlayerEntity playerIn) {
      this.syncCurrentPlayItem();
      this.field_78774_b.sendPacket(new CPlayerDiggingPacket(CPlayerDiggingPacket.Action.RELEASE_USE_ITEM, BlockPos.ZERO, Direction.DOWN));
   ...
   public void pickItem(int index) {
      this.field_78774_b.sendPacket(new CPickItemPacket(index));
   }
}