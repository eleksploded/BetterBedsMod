...
public class MegaPineTree extends HugeTreesFeature<NoFeatureConfig> {
   private static final BlockState field_181633_e = Blocks.SPRUCE_LOG.getDefaultState();
   private static final BlockState field_181634_f = Blocks.SPRUCE_LEAVES.getDefaultState();
   private static final BlockState field_181635_g = Blocks.PODZOL.getDefaultState();
   ...
   public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox p_208519_5_) {
      ...
      if (!this.func_203427_a(worldIn, position, i)) {
         ...
         for(int j = 0; j < i; ++j) {
            if (func_214572_g(worldIn, position.up(j))) {
               this.setLogState(changedBlocks, worldIn, position.up(j), this.field_76520_b, p_208519_5_);
            }

            if (j < i - 1) {
               if (func_214572_g(worldIn, position.add(1, j, 0))) {
                  this.setLogState(changedBlocks, worldIn, position.add(1, j, 0), this.field_76520_b, p_208519_5_);
               }

               if (func_214572_g(worldIn, position.add(1, j, 1))) {
                  this.setLogState(changedBlocks, worldIn, position.add(1, j, 1), this.field_76520_b, p_208519_5_);
               }

               if (func_214572_g(worldIn, position.add(0, j, 1))) {
                  this.setLogState(changedBlocks, worldIn, position.add(0, j, 1), this.field_76520_b, p_208519_5_);
               }
            }
         }
   ...
   private void placePodzolAt(IWorldGenerationReader worldIn, BlockPos pos) {
      for(int i = 2; i >= -3; --i) {
         ...
         if (isSoil(worldIn, blockpos, getSapling())) {
            this.setBlockState(worldIn, blockpos, field_181635_g);
            break;
         }

         if (!func_214574_b(worldIn, blockpos) && i < 0) {
            break;
         }
      }
