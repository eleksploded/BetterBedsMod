...
public class PointyTaigaTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
   private static final BlockState field_181636_a = Blocks.SPRUCE_LOG.getDefaultState();
   private static final BlockState field_181637_b = Blocks.SPRUCE_LEAVES.getDefaultState();
      ...
      if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getMaxHeight()) {
         ...
         if (!flag) {
                  ...
                  for(int k3 = position.getZ() - j2; k3 <= position.getZ() + j2; ++k3) {
                     ...
                     if (Math.abs(j3) != j2 || Math.abs(i2) != j2 || j2 <= 0) {
                        ...
                        if (func_214572_g(worldIn, blockpos)) {
                           this.setLogState(changedBlocks, worldIn, blockpos, field_181637_b, p_208519_5_);
                        }
                     }
                  }
               }
            ...
            for(int l2 = 0; l2 < i - 1; ++l2) {
               if (func_214572_g(worldIn, position.up(l2))) {
                  this.setLogState(changedBlocks, worldIn, position.up(l2), field_181636_a, p_208519_5_);
               }
            }
