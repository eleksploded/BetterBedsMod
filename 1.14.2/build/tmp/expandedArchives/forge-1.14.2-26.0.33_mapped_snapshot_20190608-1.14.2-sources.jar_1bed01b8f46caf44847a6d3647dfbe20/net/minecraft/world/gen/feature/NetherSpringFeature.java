...
public class NetherSpringFeature extends Feature<HellLavaConfig> {
   private static final BlockState field_214552_a = Blocks.NETHERRACK.getDefaultState();
   ...
   public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, HellLavaConfig config) {
      if (worldIn.getBlockState(pos.up()) != field_214552_a) {
         return false;
      } else if (!worldIn.getBlockState(pos).isAir(worldIn, pos) && worldIn.getBlockState(pos) != field_214552_a) {
         return false;
      } else {
         int i = 0;
         if (worldIn.getBlockState(pos.west()) == field_214552_a) {
            ++i;
         }

         if (worldIn.getBlockState(pos.east()) == field_214552_a) {
            ++i;
         }

         if (worldIn.getBlockState(pos.north()) == field_214552_a) {
            ++i;
         }

         if (worldIn.getBlockState(pos.south()) == field_214552_a) {
            ++i;
         }

         if (worldIn.getBlockState(pos.down()) == field_214552_a) {
            ++i;
         }
