...
public class LakesFeature extends Feature<LakesConfig> {
   ...
   public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, LakesConfig config) {
      ...
      if (pos.getY() <= 4) {
         ...
         if (!worldIn.func_217348_a(chunkpos.x, chunkpos.z, ChunkStatus.STRUCTURE_REFERENCES).getStructureReferences(Feature.field_214550_p.getStructureName()).isEmpty()) {
            return false;
         } else {
               ...
               for(int l2 = 0; l2 < 16; ++l2) {
                  for(int k = 0; k < 8; ++k) {
                     ...
                     if (flag) {
                        ...
                        if (k < 4 && !material.isSolid() && worldIn.getBlockState(pos.add(k1, k, l2)) != config.field_214713_a) {
                           return false;
                        }
                     }
                  }
               ...
               for(int i3 = 0; i3 < 16; ++i3) {
                  for(int i4 = 0; i4 < 8; ++i4) {
                     if (aboolean[(l1 * 16 + i3) * 8 + i4]) {
                        worldIn.setBlockState(pos.add(l1, i4, i3), i4 >= 4 ? field_205188_a : config.field_214713_a, 2);
                     }
                  }
               }
            }
            ...
            if (config.field_214713_a.getMaterial() == Material.LAVA) {
            ...
            if (config.field_214713_a.getMaterial() == Material.WATER) {
