...
public interface IChunk extends IStructureReader {
   ...
   default int getLightSubtracted(BlockPos pos, int amount, boolean hasSkylight) {
      ...
      if (worldlightmanager != null && this.getStatus().isAtLeast(ChunkStatus.LIGHT)) {
         int i = hasSkylight ? worldlightmanager.func_215569_a(LightType.SKY).func_215611_b(pos) - amount : 0;
         int j = worldlightmanager.func_215569_a(LightType.BLOCK).func_215611_b(pos);
         return Math.max(j, i);
      } else {
         return 0;
      }
