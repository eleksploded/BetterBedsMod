...
public class FrozenOceanSurfaceBuilder extends SurfaceBuilder<SurfaceBuilderConfig> {
   protected static final BlockState field_205192_a = Blocks.PACKED_ICE.getDefaultState();
   protected static final BlockState field_205193_b = Blocks.SNOW_BLOCK.getDefaultState();
   private static final BlockState field_205195_d = Blocks.AIR.getDefaultState();
   private static final BlockState field_205196_e = Blocks.GRAVEL.getDefaultState();
   private static final BlockState field_205197_f = Blocks.ICE.getDefaultState();
   ...
   public void buildSurface(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, SurfaceBuilderConfig config) {
      ...
      int k1 = x & 15;
      int i = z & 15;
      BlockState blockstate2 = biomeIn.getSurfaceBuilderConfig().getUnderMaterial();
      BlockState blockstate = biomeIn.getSurfaceBuilderConfig().getTopMaterial();
      ...
      for(int j1 = Math.max(startHeight, (int)d0 + 1); j1 >= 0; --j1) {
         ...
         if (chunkIn.getBlockState(blockpos$mutableblockpos).isAir() && j1 < (int)d0 && random.nextDouble() > 0.01D) {
            chunkIn.setBlockState(blockpos$mutableblockpos, field_205192_a, false);
         ...
         if (blockstate1.isAir()) {
            ...
            if (blockstate1.getBlock() == Blocks.PACKED_ICE && k <= l && j1 > i1) {
               chunkIn.setBlockState(blockpos$mutableblockpos, field_205193_b, false);
               ++k;
            }
         } else if (j == -1) {
            if (l1 <= 0) {
               blockstate = field_205195_d;
               blockstate2 = defaultBlock;
            } else if (j1 >= seaLevel - 4 && j1 <= seaLevel + 1) {
               blockstate = biomeIn.getSurfaceBuilderConfig().getTopMaterial();
               blockstate2 = biomeIn.getSurfaceBuilderConfig().getUnderMaterial();
            }

            if (j1 < seaLevel && (blockstate == null || blockstate.isAir())) {
               if (biomeIn.getTemperature(blockpos$mutableblockpos.setPos(x, j1, z)) < 0.15F) {
                  blockstate = field_205197_f;
               } else {
            ...
            if (j1 >= seaLevel - 1) {
               ...
               blockstate = field_205195_d;
               blockstate2 = defaultBlock;
               chunkIn.setBlockState(blockpos$mutableblockpos, field_205196_e, false);
            } else {
