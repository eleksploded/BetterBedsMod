...
public class BambooFeature extends Feature<ProbabilityConfig> {
   private static final BlockState field_214566_a = Blocks.field_222405_kQ.getDefaultState().with(BambooBlock.field_220264_d, Integer.valueOf(1)).with(BambooBlock.field_220265_e, BambooLeaves.NONE).with(BambooBlock.field_220266_f, Integer.valueOf(0));
   ...
   public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, ProbabilityConfig config) {
      ...
      if (worldIn.isAirBlock(blockpos$mutableblockpos)) {
         if (Blocks.field_222405_kQ.getDefaultState().isValidPosition(worldIn, blockpos$mutableblockpos)) {
               ...
               for(int l = pos.getX() - k; l <= pos.getX() + k; ++l) {
                  for(int i1 = pos.getZ() - k; i1 <= pos.getZ() + k; ++i1) {
                     ...
                     if (j1 * j1 + k1 * k1 <= k * k) {
                        ...
                        if (worldIn.getBlockState(blockpos$mutableblockpos1).getBlock().isIn(BlockTags.field_219750_S)) {
