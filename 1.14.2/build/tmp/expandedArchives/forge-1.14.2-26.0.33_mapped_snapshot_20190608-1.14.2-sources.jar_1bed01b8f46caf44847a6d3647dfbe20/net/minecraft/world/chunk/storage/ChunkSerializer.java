...
public class ChunkSerializer {
   ...
   public static ChunkPrimer read(ServerWorld worldIn, TemplateManager p_222656_1_, PointOfInterestManager p_222656_2_, ChunkPos pos, CompoundNBT compound) {
      ...
      if (compoundnbt.contains("Biomes", 11)) {
         ...
         for(int i = 0; i < aint.length; ++i) {
            abiome[i] = Registry.field_212624_m.getByValue(aint[i]);
            if (abiome[i] == null) {
      ...
      if (flag) {
         worldlightmanager.func_223115_b(pos, true);
      }
      ...
      for(int k = 0; k < listnbt.size(); ++k) {
         ...
         if (compoundnbt1.contains("Palette", 9) && compoundnbt1.contains("BlockStates", 12)) {
            ...
            chunksection.getData().func_222642_a(compoundnbt1.getList("Palette", 10), compoundnbt1.getLongArray("BlockStates"));
         ...
         if (flag) {
            if (compoundnbt1.contains("BlockLight", 7)) {
               worldlightmanager.func_215574_a(LightType.BLOCK, SectionPos.from(pos, l), new NibbleArray(compoundnbt1.getByteArray("BlockLight")));
            }

            if (flag1 && compoundnbt1.contains("SkyLight", 7)) {
               worldlightmanager.func_215574_a(LightType.SKY, SectionPos.from(pos, l), new NibbleArray(compoundnbt1.getByteArray("SkyLight")));
            }
         }
      }
      ...
      if (chunkstatus$type == ChunkStatus.Type.LEVELCHUNK) {
         ...
         if (compoundnbt.contains("TileTicks", 9)) {
            iticklist = SerializableTickList.func_222984_a(compoundnbt.getList("TileTicks", 10), Registry.field_212618_g::getKey, Registry.field_212618_g::getOrDefault);
         } else {
         ...
         if (compoundnbt.contains("LiquidTicks", 9)) {
            iticklist1 = SerializableTickList.func_222984_a(compoundnbt.getList("LiquidTicks", 10), Registry.field_212619_h::getKey, Registry.field_212619_h::getOrDefault);
         } else {
      ...
      for(Heightmap.Type heightmap$type : ichunk.getStatus().func_222595_h()) {
   ...
   public static CompoundNBT write(ServerWorld worldIn, IChunk chunkIn) {
      ...
      for(int i = -1; i < 17; ++i) {
         ...
         NibbleArray nibblearray = worldlightmanager.func_215569_a(LightType.BLOCK).func_215612_a(SectionPos.from(chunkpos, i));
         NibbleArray nibblearray1 = worldlightmanager.func_215569_a(LightType.SKY).func_215612_a(SectionPos.from(chunkpos, i));
      ...
      if (abiome != null) {
         for(int j = 0; j < abiome.length; ++j) {
            aint[j] = Registry.field_212624_m.getId(abiome[j]);
         }
      }
      ...
      for(Entry<Heightmap.Type, Heightmap> entry : chunkIn.func_217311_f()) {
         if (chunkIn.getStatus().func_222595_h().contains(entry.getKey())) {
