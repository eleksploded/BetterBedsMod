...
public class PillagerOutpostStructure extends ScatteredStructure<PillagerOutpostConfig> {
   private static final List<Biome.SpawnListEntry> field_214558_a = Lists.newArrayList(new Biome.SpawnListEntry(EntityType.field_220350_aJ, 1, 1, 1));
   ...
   public boolean hasStartAt(ChunkGenerator<?> chunkGen, Random rand, int chunkPosX, int chunkPosZ) {
      ...
      if (chunkPosX == chunkpos.x && chunkPosZ == chunkpos.z) {
         ...
         if (chunkGen.hasStructure(biome, Feature.field_214536_b)) {
            for(int k = chunkPosX - 10; k <= chunkPosX + 10; ++k) {
               for(int l = chunkPosZ - 10; l <= chunkPosZ + 10; ++l) {
                  if (Feature.field_214550_p.hasStartAt(chunkGen, rand, k, l)) {
                     return false;
                  }
               }
            }
