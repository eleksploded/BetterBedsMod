...
public class DoublePlantFeature extends Feature<DoublePlantConfig> {
   ...
   public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand, BlockPos pos, DoublePlantConfig config) {
      ...
      for(int i = 0; i < 64; ++i) {
         ...
         if (worldIn.isAirBlock(blockpos) && blockpos.getY() < worldIn.getWorld().getDimension().getHeight() - 2 && config.field_202435_a.isValidPosition(worldIn, blockpos)) {
            ((DoublePlantBlock)config.field_202435_a.getBlock()).placeAt(worldIn, blockpos, 2);
            flag = true;
         }
      }
