         ...
         for(Template.BlockInfo template$blockinfo : this.template.func_215381_a(this.templatePosition, this.placeSettings, Blocks.STRUCTURE_BLOCK)) {
            if (template$blockinfo.tileentityData != null) {
               ...
               if (structuremode == StructureMode.DATA) {
                  this.handleDataMarker(template$blockinfo.tileentityData.getString("metadata"), template$blockinfo.pos, worldIn, randomIn, structureBoundingBoxIn);
               }
            }
         }

         for(Template.BlockInfo template$blockinfo1 : this.template.func_215381_a(this.templatePosition, this.placeSettings, Blocks.field_222435_lY)) {
            if (template$blockinfo1.tileentityData != null) {
               String s = template$blockinfo1.tileentityData.getString("final_state");
