...
public class SwampTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
   private static final BlockState field_181648_a = Blocks.OAK_LOG.getDefaultState();
   private static final BlockState field_181649_b = Blocks.OAK_LEAVES.getDefaultState();
      ...
      if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getMaxHeight()) {
            ...
            for(int l = position.getX() - k; l <= position.getX() + k && flag; ++l) {
               for(int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; ++i1) {
                  if (j >= 0 && j < worldIn.getMaxHeight()) {
                     ...
                     if (!func_214572_g(worldIn, blockpos$mutableblockpos)) {
                        if (func_214571_e(worldIn, blockpos$mutableblockpos)) {
                           if (j > position.getY()) {
                              flag = false;
                           }
                        } else {
         ...
         if (!flag) {
               ...
               for(int k3 = position.getX() - i3; k3 <= position.getX() + i3; ++k3) {
                  ...
                  for(int j1 = position.getZ() - i3; j1 <= position.getZ() + i3; ++j1) {
                     ...
                     if (Math.abs(l3) != i3 || Math.abs(k1) != i3 || rand.nextInt(2) != 0 && k2 != 0) {
                        ...
                        if (func_214572_g(worldIn, blockpos) || func_214576_j(worldIn, blockpos)) {
                           this.setLogState(changedBlocks, worldIn, blockpos, field_181649_b, p_208519_5_);
                        }
                     }
                  }
               }
            ...
            for(int i2 = 0; i2 < i; ++i2) {
               ...
               if (func_214572_g(worldIn, blockpos3) || func_214571_e(worldIn, blockpos3)) {
                  this.setLogState(changedBlocks, worldIn, blockpos3, field_181648_a, p_208519_5_);
               }
            }
            ...
            for(int j2 = position.getY() - 3 + i; j2 <= position.getY() + i; ++j2) {
               ...
               for(int i4 = position.getX() - j3; i4 <= position.getX() + j3; ++i4) {
                  for(int j4 = position.getZ() - j3; j4 <= position.getZ() + j3; ++j4) {
                     ...
                     if (func_214570_f(worldIn, blockpos$mutableblockpos1)) {
                        BlockPos blockpos4 = blockpos$mutableblockpos1.west();
                        BlockPos blockpos5 = blockpos$mutableblockpos1.east();
                        BlockPos blockpos1 = blockpos$mutableblockpos1.north();
                        BlockPos blockpos2 = blockpos$mutableblockpos1.south();
                        if (rand.nextInt(4) == 0 && func_214574_b(worldIn, blockpos4)) {
                           this.addVine(worldIn, blockpos4, VineBlock.EAST);
                        }

                        if (rand.nextInt(4) == 0 && func_214574_b(worldIn, blockpos5)) {
                           this.addVine(worldIn, blockpos5, VineBlock.WEST);
                        }

                        if (rand.nextInt(4) == 0 && func_214574_b(worldIn, blockpos1)) {
                           this.addVine(worldIn, blockpos1, VineBlock.SOUTH);
                        }

                        if (rand.nextInt(4) == 0 && func_214574_b(worldIn, blockpos2)) {
   ...
   private void addVine(IWorldGenerationReader worldIn, BlockPos pos, BooleanProperty prop) {
      ...
      for(BlockPos blockpos = pos.down(); func_214574_b(worldIn, blockpos) && i > 0; --i) {
