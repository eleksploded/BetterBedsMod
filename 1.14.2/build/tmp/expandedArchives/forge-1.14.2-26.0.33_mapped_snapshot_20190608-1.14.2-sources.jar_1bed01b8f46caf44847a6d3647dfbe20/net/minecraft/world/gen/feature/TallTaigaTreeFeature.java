...
public class TallTaigaTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
   private static final BlockState field_181645_a = Blocks.SPRUCE_LOG.getDefaultState();
   private static final BlockState field_181646_b = Blocks.SPRUCE_LEAVES.getDefaultState();
      ...
      if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getMaxHeight()) {
         ...
         if (!flag) {
                  ...
                  for(int k2 = position.getZ() - i3; k2 <= position.getZ() + i3; ++k2) {
                     ...
                     if (Math.abs(j2) != i3 || Math.abs(l2) != i3 || i3 <= 0) {
                        ...
                        if (func_214572_g(worldIn, blockpos) || func_214576_j(worldIn, blockpos)) {
                           this.setLogState(changedBlocks, worldIn, blockpos, field_181646_b, p_208519_5_);
                        }
                     }
                  }
               }
            ...
            for(int k4 = 0; k4 < i - i4; ++k4) {
               if (func_214572_g(worldIn, position.up(k4))) {
                  this.setLogState(changedBlocks, worldIn, position.up(k4), field_181645_a, p_208519_5_);
               }
            }
