...
public class BowItem extends ShootableItem {
   ...
   public void onPlayerStoppedUsing(ItemStack stack, World worldIn, LivingEntity entityLiving, int timeLeft) {
      if (entityLiving instanceof PlayerEntity) {
         ...
         boolean flag = playerentity.playerAbilities.isCreativeMode || EnchantmentHelper.getEnchantmentLevel(Enchantments.INFINITY, stack) > 0;
         ...
         if (!itemstack.isEmpty() || flag) {
            ...
            if (!((double)f < 0.1D)) {
               boolean flag1 = playerentity.playerAbilities.isCreativeMode || (itemstack.getItem() instanceof ArrowItem && ((ArrowItem)itemstack.getItem()).isInfinite(itemstack, stack, playerentity));
               if (!worldIn.isRemote) {
                  ...
                  stack.func_222118_a(1, playerentity, (p_220009_1_) -> {
                     p_220009_1_.func_213334_d(playerentity.getActiveHand());
                  });
                  if (flag1 || playerentity.playerAbilities.isCreativeMode && (itemstack.getItem() == Items.SPECTRAL_ARROW || itemstack.getItem() == Items.TIPPED_ARROW)) {
                     abstractarrowentity.field_70251_a = AbstractArrowEntity.PickupStatus.CREATIVE_ONLY;
                  }

                  worldIn.func_217376_c(abstractarrowentity);
               }
               ...
               if (!flag1 && !playerentity.playerAbilities.isCreativeMode) {
                  itemstack.shrink(1);
                  if (itemstack.isEmpty()) {
   ...
   public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
      ...
      if (!playerIn.playerAbilities.isCreativeMode && !flag) {
