...
public class ServerRecipePlacer<C extends IInventory> implements IRecipePlacer<Integer> {
   ...
   public ServerRecipePlacer(RecipeBookContainer<C> p_i50752_1_) {
      this.field_201515_d = p_i50752_1_;
   }
   ...
   protected void clear() {
      for(int i = 0; i < this.field_201515_d.getWidth() * this.field_201515_d.getHeight() + 1; ++i) {
         if (i != this.field_201515_d.getOutputSlot() || !(this.field_201515_d instanceof WorkbenchContainer) && !(this.field_201515_d instanceof PlayerContainer)) {
            this.giveToPlayer(i);
         }
      }

      this.field_201515_d.clear();
   }

   protected void giveToPlayer(int slotIn) {
      ItemStack itemstack = this.field_201515_d.getSlot(slotIn).getStack();
      if (!itemstack.isEmpty()) {
         for(; itemstack.getCount() > 0; this.field_201515_d.getSlot(slotIn).decrStackSize(1)) {
            int i = this.field_201514_c.storeItemStack(itemstack);
            if (i == -1) {
               i = this.field_201514_c.getFirstEmptyStack();
            }

            ItemStack itemstack1 = itemstack.copy();
            itemstack1.setCount(1);
            if (!this.field_201514_c.add(i, itemstack1)) {
   ...
   protected void tryPlaceRecipe(IRecipe<C> p_201508_1_, boolean placeAll) {
      boolean flag = this.field_201515_d.matches(p_201508_1_);
      int i = this.recipeItemHelper.getBiggestCraftableStack(p_201508_1_, (IntList)null);
      if (flag) {
         for(int j = 0; j < this.field_201515_d.getHeight() * this.field_201515_d.getWidth() + 1; ++j) {
            if (j != this.field_201515_d.getOutputSlot()) {
               ItemStack itemstack = this.field_201515_d.getSlot(j).getStack();
      ...
      if (this.recipeItemHelper.canCraft(p_201508_1_, intlist, j1)) {
         ...
         if (this.recipeItemHelper.canCraft(p_201508_1_, intlist, k)) {
            this.clear();
            this.placeRecipe(this.field_201515_d.getWidth(), this.field_201515_d.getHeight(), this.field_201515_d.getOutputSlot(), p_201508_1_, intlist.iterator(), k);
         }
      }
   ...
   public void setSlotContents(Iterator<Integer> ingredients, int slotIn, int maxAmount, int y, int x) {
      Slot slot = this.field_201515_d.getSlot(slotIn);
   ...
   protected int getMaxAmount(boolean placeAll, int maxPossible, boolean recipeMatches) {
      ...
      if (placeAll) {
         ...
         for(int j = 0; j < this.field_201515_d.getWidth() * this.field_201515_d.getHeight() + 1; ++j) {
            if (j != this.field_201515_d.getOutputSlot()) {
               ItemStack itemstack = this.field_201515_d.getSlot(j).getStack();
   ...
   protected void consumeIngredient(Slot slotToFill, ItemStack ingredientIn) {
      int i = this.field_201514_c.findSlotMatchingUnusedItem(ingredientIn);
      if (i != -1) {
         ItemStack itemstack = this.field_201514_c.getStackInSlot(i).copy();
         if (!itemstack.isEmpty()) {
            if (itemstack.getCount() > 1) {
               this.field_201514_c.decrStackSize(i, 1);
            } else {
               this.field_201514_c.removeStackFromSlot(i);
            }
   ...
   private boolean func_194328_c() {
      ...
      for(int j = 0; j < this.field_201515_d.getWidth() * this.field_201515_d.getHeight() + 1; ++j) {
         if (j != this.field_201515_d.getOutputSlot()) {
            ItemStack itemstack = this.field_201515_d.getSlot(j).getStack().copy();
            if (!itemstack.isEmpty()) {
               int k = this.field_201514_c.storeItemStack(itemstack);
               if (k == -1 && list.size() <= i) {
   ...
   private int getEmptyPlayerSlots() {
      ...
      for(ItemStack itemstack : this.field_201514_c.mainInventory) {
         if (itemstack.isEmpty()) {
            ++i;
         }
      }
